...
public abstract class AppTemplate extends Application {
    ...
    public void start(Stage primaryStage) {
        if(!loadLanguageJson()){
            ...
            APP_PROPERTIES_FILE_NAME = APP_PROPERTIES_FILE_NAME.substring(1, APP_PROPERTIES_FILE_NAME.length()-1);
            changeLanguage(APP_PROPERTIES_FILE_NAME);
        }
	...
	try {
		...
		gui = new AppGUI(primaryStage, appTitle, this);
                gui.getFileController().writeLanguageJson();
     ...
     * And set the APP_PROPERTIES_FILE_NAME if the file exist.
     * Homework 2
     */
    public boolean loadLanguageJson(){
        try{
            // File exist
            InputStream is = new FileInputStream(PATH_LANGUAGE);
            JsonReader jsonFileReader = Json.createReader(is);
            JsonObject obj = jsonFileReader.readObject();
            APP_PROPERTIES_FILE_NAME = obj.getJsonString("language").toString();
            return true;
        }
